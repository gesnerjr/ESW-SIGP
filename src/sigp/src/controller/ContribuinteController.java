...
public class ContribuinteController implements IHeaderController {
	...
	public ContribuinteController(Result result, Validator validator,
			MembroDao dao, UsuarioDao udao, NoticiaDao ndao,
			MembroBusiness business) {
		this.result = result;
	...
	@Post
	@Path("/contribuinte/ajax")
	public void ajax(final Contribuinte contribuinte, final Long idUsuario)
			throws IOException {
	...
	public void cria(final Contribuinte contribuinte, final Long idUsuario,
		...
		if (file == null) {
			...
			result.redirectTo(ContribuinteController.class).resize_image(
